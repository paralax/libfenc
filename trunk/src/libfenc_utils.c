...
encode_plaintext_GT(fenc_plaintext* plaintext, element_t *plaintextT, pairing_t pairing)
{
	...
	if (plaintext_len > pairing_length_in_bytes_Zr(pairing)) {
		return FENC_ERROR_BUFFER_TOO_SMALL;
	}
...
{
	uint8 *serialized_elt;
	size_t element_len;
	
	return hash_element_to_bytes(element, key_bytes, key_buf, HASH_FUNCTION_KEM_DERIVE);
}
